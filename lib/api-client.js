...
class LearnPressAPIClient {
    constructor(apiUrl, deviceId) {
        this.apiUrl = apiUrl.replace(/^http:\/\//i, 'https://').replace(/\/$/, '');
    ...
    logAuthEvent(event, data = {}) {
        ...
        console.log(`[API-AUTH] ${event}:`, logEntry);
        
        // Sauvegarder dans un fichier si en mode debug
    ...
    handleApiError(error, context = {}) {
        ...
        if (error.type === 'http_error' && error.status) {
            errorInfo.type = 'response';
            errorInfo.status = error.status;
            errorInfo.data = error.data;
            errorInfo.userMessage = error.data?.message || error.message;
        ...
        if (error.type === 'network_error') {
            ...
            errorInfo.userMessage = error.message || 'Impossible de contacter le serveur.';
    ...
    async login(username, password) {
        try {
            ...
            // Si c'est déjà un objet errorInfo (from handleApiError or normalizeError)
            if (error.userMessage || error.type === 'http_error') {
                const serverMessage = error.data?.message || error.userMessage || error.message;
                return {
                    success: false,
                    error: serverMessage,
                    code: error.data?.code || error.code,
                    status: error.status,
                    requiresMembership: ['no_active_membership', 'no_membership'].includes(error.data?.code || error.code)
                };
            }
            ...
            if (error.response) {
                ...
                switch (status) {
                    ...
                    case 401:
                        ...
                        break;
                    case 403:
                        if (data.code === 'no_active_membership' || data.code === 'no_membership') {
                ...
                return {
                    ...
                    code: errorCode,
                    status: status,
                    requiresMembership: ['no_active_membership', 'no_membership'].includes(errorCode)
                };
            }
            ...
            if (error.userMessage) {
                if (error.requiresReauth) {
                    ...
                    if (typeof this.onAuthFailure === 'function') {
                        this.onAuthFailure('force-logout', {
    ...
    // Rafraîchir le token
    // Rafraîchir le token - VERSION AMÉLIORÉE
    async refreshAccessToken() {
        ...
        while (retries > 0) {
            try {
                console.log(`[API] Tentative de refresh token (${4 - retries}/3)...`);
                ...
                retries--;
                console.error(`[API] Erreur refresh token, ${retries} essais restants:`, error.message);
...
// Ajouter cette classe à la fin du fichier api-client.js
class AutoReconnect {
    ...
    async getSavedCredentials() {
        try {
            const apiUrl = this.store.get('savedApiUrl');
            const username = this.store.get('savedUsername');
            // Note: Le mot de passe devrait être stocké de manière sécurisée
            const encryptedPassword = this.store.get('savedPasswordHash');
            
            if (!apiUrl || !username || !encryptedPassword) {
                return null;
            }
            
            return { apiUrl, username, encryptedPassword };
        } catch (error) {
            console.error('[AutoReconnect] Erreur récupération credentials:', error);
            return null;
        }
    }
    ...
    async tryReconnect() {
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
            console.log('[AutoReconnect] Nombre maximum de tentatives atteint');
            return false;
        }
        
        const savedCredentials = await this.getSavedCredentials();
        if (!savedCredentials) {
            console.log('[AutoReconnect] Pas de credentials sauvegardés');
            return false;
        }
        
        try {
            console.log(`[AutoReconnect] Tentative ${this.reconnectAttempts + 1}/${this.maxReconnectAttempts}`);
            
            // Ici, vous devriez implémenter un déchiffrement sécurisé du mot de passe
            // Pour l'exemple, on suppose qu'il est déchiffré
            
            const result = await this.apiClient.login(
                savedCredentials.apiUrl,
                savedCredentials.username,
                savedCredentials.password // À déchiffrer
            );
            ...
            console.error('[AutoReconnect] Erreur de reconnexion:', error);
            this.reconnectAttempts++;
        }
...
module.exports = LearnPressAPIClient;
module.exports.AutoReconnect = AutoReconnect;