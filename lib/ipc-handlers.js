...
ipcMain.handle('check-auto-login', async () => {
    try {
        ...
        if (token && tokenExpiry && Date.now() < tokenExpiry) {
            ...
            apiClient.onAuthFailure = (eventName, data) => {
        ...
        if (refreshToken) {
            try {
                ...
                if (refreshResult.success) {
                    ...
                    const expiresInMs = (refreshResult.expiresIn || 3600) * 1000;
                    store.set('tokenExpiry', Date.now() + expiresInMs);
  ...
  ipcMain.handle('api-login', async (event, { apiUrl, username, password }) => {
    try {
        ...
        if (result.success) {
            ...
            store.set('tokenExpiry', Date.now() + (result.expiresIn || 3600) * 1000);
...
ipcMain.handle('download:removeFromHistory', async (event, downloadId) => {
    try {
        const downloadManager = getDownloadManager();
        if (!downloadManager) {
            return { success: false, error: 'Download manager non initialisé' };
        }
        if (typeof downloadManager.removeFromHistory === 'function') {
            return await downloadManager.removeFromHistory(downloadId);
        }
        return { success: true };
    } catch (error) {
        console.error('Erreur remove from history:', error);
        return { success: false, error: error.message };
    }
});
...
ipcMain.handle('db-update-lesson-progress', async (event, payload = {}) => {
  ...
  ipcMain.handle('log-error', async (event, error) => {
    ...
    log.error('[Renderer Error]', error.message || error);
    return { success: true };
  });

  ipcMain.handle('report-error', async (event, error) => {
    const log = require('electron-log');
    log.error('[Renderer Report]', error.message || error);
    return { success: true };
  });
...
ipcMain.handle('media:createStreamUrl', async (event, encryptedPath, mimeType) => {
  try {
    ...
    if (secureMediaPlayer._lastStreamId) {
      secureMediaPlayer.removeStream(secureMediaPlayer._lastStreamId);
    }

    const streamUrl = await secureMediaPlayer.createStreamUrl(encryptedPath, mimeType);
    const streamId = streamUrl.split('/stream/')[1];
    secureMediaPlayer._lastStreamId = streamId;
...
ipcMain.handle('media:closeStream', async (event, streamUrl) => {
  try {
    const { getSecureMediaPlayer } = context;
    const secureMediaPlayer = await getSecureMediaPlayer();
    const streamId = streamUrl?.split('/stream/')[1];
    if (streamId) {
      secureMediaPlayer.removeStream(streamId);
    }
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
