...
function setupIpcHandlers(ipcMain, context) {
  ...
  ipcMain.handle('check-auto-login', async () => {
    try {
        ...
        // IMPORTANT : Définir les tokens dans le client
        if (token) {
        ...
        // Si on a un refresh token, essayer le refresh
        if (refreshToken) {
            try {
                console.log('[IPC] Tentative de refresh...');
                const refreshResult = await apiClient.refreshAccessToken();
                
                if (refreshResult.success) {
                    console.log('[IPC] Refresh réussi, nouveau token:', apiClient.token ? 'présent' : 'absent');
                    ...
                    // IMPORTANT : Définir le client API global
                    setApiClient(apiClient);
                    console.log('[IPC] Client API défini avec token');
        ...
        // Si on a seulement un token valide, l'utiliser directement
        if (token) {
            try {
                console.log('[IPC] Utilisation du token existant...');
                
                // IMPORTANT : Définir le client API global
                setApiClient(apiClient);
                console.log('[IPC] Client API défini avec token existant');
                
                const result = await apiClient.verifySubscription();
                
                if (result.success) {
                    console.log('[IPC] Token valide');
