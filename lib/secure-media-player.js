...
class SecureMediaPlayer {
    ...
    handleGCMStream(req, res, stream) {
        ...
        // Wrap the GCM decipher to suppress the final() authTag error.
        // GCM without authTag throws on _flush, but all data from update()
        // has already been passed through. We catch the flush error and end cleanly.
        const safeDecipher = new Transform({
            ...
            flush(callback) {
                try {
                    ...
                    // Expected: GCM authTag verification fails (no authTag stored)
                    console.log('[SecureMediaPlayer] GCM flush completed (no authTag, expected)');
                }
                callback();
            }
        });
        ...
        safeDecipher.on('error', (err) => {
            console.error(`[SecureMediaPlayer] GCM decrypt error: ${err.message}`);
            if (!res.headersSent) res.sendStatus(500);
            else res.end();
        });

        fileStream.pipe(safeDecipher).pipe(res);

        res.on('close', () => {
            fileStream.destroy();
            safeDecipher.destroy();
        });
    }
