...
class ErrorHandler {
    ...
    // Déterminer le type d'erreur (matching strict par code uniquement)
    determineErrorType(error) {
        const code = error.code || '';
        
        if (code === 'AUTH_ERROR' || code === 'INVALID_CREDENTIALS' || code === 'TOKEN_EXPIRED') {
            return 'AUTH_ERROR';
        }
        if (code === 'ENOTFOUND' || code === 'ETIMEDOUT' || code === 'ECONNREFUSED' || code === 'ECONNRESET' || code === 'NETWORK_ERROR') {
            return 'NETWORK_ERROR';
        }
        if (code === 'SQLITE_ERROR' || code === 'SQLITE_CORRUPT' || code === 'SQLITE_BUSY') {
            return 'DATABASE_ERROR';
        }
        if (code === 'ENOSPC' || code === 'EACCES' || code === 'EPERM' || code === 'STORAGE_ERROR') {
            return 'STORAGE_ERROR';
        }
        if (code === 'NO_MEMBERSHIP' || code === 'MEMBERSHIP_EXPIRED' || code === 'MEMBERSHIP_ERROR') {
            return 'MEMBERSHIP_ERROR';
        }
        if (code === 'SYNC_ERROR' || code === 'SYNC_CONFLICT') {
            return 'SYNC_ERROR';
        }
        if (code === 'DOWNLOAD_ERROR' || code === 'DOWNLOAD_FAILED') {
            return 'DOWNLOAD_ERROR';
        }
        if (code === 'ENCRYPTION_ERROR' || code === 'DECRYPT_FAILED' || code === 'KEY_MISSING') {
            return 'ENCRYPTION_ERROR';
        }
        if (code === 'UPDATE_ERROR' || code === 'UPDATE_FAILED') {
            return 'UPDATE_ERROR';
        }
