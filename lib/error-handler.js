...
class ErrorHandler {
        ...
        this.errorMessages = {
            fr: {
                AUTH_ERROR: {
                    ...
                    GENERIC: 'Erreur d\'authentification. Veuillez vous reconnecter.'
                },
                NETWORK_ERROR: {
    ...
    async handleError(error, context = {}) {
        ...
        // Ne pas afficher de dialogue bloquant pour les erreurs
        // non-critiques venant du renderer ou des rejections non gérées
        const silentSources = ['renderer', 'unhandledRejection'];
        const silentTypes = ['UNKNOWN_ERROR', 'UPDATE_ERROR', 'SYNC_ERROR'];
        if (silentSources.includes(context.source || context.type) && silentTypes.includes(errorType)) {
            log.warn(`Erreur silencieuse (${errorType}):`, error.message || error);
    ...
    determineErrorType(error) {
        ...
        if (code === 'AUTH_ERROR' || code === 'INVALID_CREDENTIALS' ||
            code === 'TOKEN_EXPIRED' || code === 'NO_MEMBERSHIP' ||
            code === 'MEMBERSHIP_EXPIRED') {
            return 'AUTH_ERROR';
        }
        if (code === 'ENOTFOUND' || code === 'ETIMEDOUT' || code === 'ERR_NETWORK' ||
            msg.includes('network') || msg.includes('connexion réseau')) {
            return 'NETWORK_ERROR';
        }
        if (code === 'SQLITE_ERROR' || msg.includes('database') || msg.includes('base de données')) {
            return 'DATABASE_ERROR';
        }
        if (code === 'ENOSPC' || code === 'EACCES' || msg.includes('storage') || msg.includes('espace')) {
            return 'STORAGE_ERROR';
        }
        if (code === 'NO_MEMBERSHIP' || code === 'MEMBERSHIP_ERROR' || msg === 'membership') {
            return 'MEMBERSHIP_ERROR';
        }
        if (code === 'SYNC_ERROR' || msg === 'sync') {
            return 'SYNC_ERROR';
        }
        if (code === 'DOWNLOAD_ERROR' || msg === 'download') {
            return 'DOWNLOAD_ERROR';
        }
        if (msg.includes('encrypt') || msg.includes('decrypt') || msg.includes('chiffr')) {
